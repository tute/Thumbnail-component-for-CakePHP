...
* @images_size@: Array of different file sizes required by your app. Each
  element is itself an array like like the following one:
    @'folder_name' => array($width, $height, $do_crop)@.

Yo may override the default configuration by passing an array of options
while including the component:
@var $components = array('Attachment' => array(/* options */));@

(You may check the @$config@ array defined at the first lines of the
component, to see default values and the syntax for overriding them.)
