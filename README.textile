*Attachment component for CakePHP* handles file uploads to the file system. It
has thumbnailing capabilities if it's an image file.
...
# In your controller action:
	@$img_path = $this->Attachment->upload($this->data['Model']['Attachment']);@
# In your upload form:
## @echo $form->create('Model', array('type' => 'file'));@
## @echo $form->file('Attachment');@
# In your view, if it's an image (@$data['Model']['image_path']@):
	@<?= $html->image('/attachments/$folderName/big/'.$data['Model']['image_path']); ?>@

h2. Parameters:

h3. upload($data)

* @data@: file data array from the form
